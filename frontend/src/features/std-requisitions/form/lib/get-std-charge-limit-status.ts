...
export function getStdChargeLimitStatus(
    ...
    if (row.chargeType === STD_CHARGE_TYPE.Mileage) {
        ...
        if ((row.miles ?? 0) > mileageLimitRule.maxQuantity) {
            messages.push(`Miles exceed maximum of ${mileageLimitRule.maxQuantity}.`);
        }
